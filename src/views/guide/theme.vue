...
<template>
    <i-article>
        <article>
            ...
            <i-code :title="title" lang="auto" bg>{{ code.theme.less }}</i-code>
            <p>
            ...
            <i-code :title="title" lang="auto" bg>{{ code.theme.lessImport }}</i-code>
            ...
            <i-code :title="title" lang="auto" bg>{{ code.theme.install }}</i-code>
            <p>然后在业务工程里新建一个目录，用来存放主题文件，使用下面的命令初始化主题，这时会从 iView 仓库拉取最新的样式文件：</p>
            <i-code :title="title" lang="auto" bg>{{ code.theme.init }}</i-code>
            <p>最后编辑 <code>my-theme/custom.less</code> 文件，用命令编译即可：</p>
            <i-code :title="title" lang="auto" bg>{{ code.theme.build }}</i-code>
            <p>最终会在指定的目录下编译为 <code>iview.css</code> 的文件，只需在入口处引用它就可以了，比如在 main.js 文件：</p>
            <i-code :title="title" lang="js" bg>{{ code.theme.import }}</i-code>
